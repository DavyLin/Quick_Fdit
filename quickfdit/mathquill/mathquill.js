...
var SupSub = P(MathCommand, function(_, _super) {
  ...
  _.deleteOutOf=function(){console.log("del")};
  ...
  _.respace = function() {
	...
	//sum, prod: ^ and _ acts up and down.
    else if (
      this[L].ctrlSeq === '\\sum ' || (
        this[L] instanceof SupSub && this[L].ctrlSeq != this.ctrlSeq
        && this[L][L] && this[L][L].ctrlSeq === '\\sum '
      )
    ) {
      if (!this.ud) {
        this.ud = true;
      ...
      this[L].ctrlSeq === '\\prod ' || (
        this[L] instanceof SupSub && this[L].ctrlSeq != this.ctrlSeq
        && this[L][L] && this[L][L].ctrlSeq === '\\prod '
      )
    ) {
      if (!this.ud) {
        this.ud = true;
    ...
    if (this.respaced) {
	  ...
	  if (this.ud){//add:, there must be an [L][L].
	    var leftleftWidth=this[L][L].jQ.outerWidth();
		if (leftleftWidth>leftWidth){
          this.jQ.css({
            left: -(leftleftWidth+thisWidth)/2 + 'px',
            marginRight: .1 - min(thisWidth, (leftleftWidth+thisWidth)/2) + 'px'
            //1px extra so it doesn't wrap in retarded browsers (Firefox 2, I think)
          });
	      //---
		  this[L][L].jQ.css({
		    marginLeft:max(0,thisWidth-leftleftWidth)/2 + 'px'
		  });
		}
		else{
          this.jQ.css({
            left: -(leftWidth+thisWidth)/2 + 'px',
            marginRight: .1 - min(thisWidth, (leftWidth+thisWidth)/2) + 'px'
            //1px extra so it doesn't wrap in retarded browsers (Firefox 2, I think)
          });
		  //---
		  this[L][L].jQ.css({
		    marginLeft:max(leftWidth-leftleftWidth,thisWidth-leftleftWidth)/2 + 'px'
		  });
		}
	    //console.log(thisWidth+' '+ leftWidth+' '+this.latex());
      }
	  else //original:
        this.jQ.css({
	  ...
	  this[L].jQ.css({
	    marginLeft:max(0,thisWidth-leftWidth)/2 + 'px'
	  })
	}
...
var Bracket = P(MathCommand, function(_, _super) {
  _.init = function(open, close, ctrlSeq, end) {
    _super.init.call(this, '\\left'+ctrlSeq,
      ...
      +   '<span class="non-leaf" style="vertical-align:middle">&0</span>'
